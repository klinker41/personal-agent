...
`0600` on private keys) and matching container UID ownership, silently failing
key discovery if created by root or with broad permissions.
- Setting `StrictHostKeyChecking accept-new` in `~/.ssh/config` or
pre-populating known host keys avoids interactive verification failures during
non-interactive container SSH and Git operations.
