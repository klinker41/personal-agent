...
updated_at: 2026-09-18T00:40:32.557810+00:00
confidence: 0.95
---
...
- **Headless Initialization & Stepping**: PyBoy 2.0+ requires `window="null"`
  for headless execution (superseding PyBoy 1.x `window_type="dummy"`). Advance
  synchronously via `pyboy.tick(1, render=True, sound=True)` with
  `sound_emulated=True` and speed set to 1 to lock real-time wall-clock pacing
  (~59.73 fps) and prevent unthrottled CPU execution. Cold boots require
  initial PPU execution ticks before valid framebuffer data is available.

- **A/V Media Extraction & Signal Conditioning**: Screen capture yields a
  144x160x4 RGBA framebuffer (native 160x144 Game Boy resolution, producing
  exactly 69,120 bytes of raw RGB24 video per frame for streaming). Emulated
  audio requires a `sound_sample_rate` evenly divisible by 60 (e.g., 44100 or
  48000), scaling the native `int8` buffer to 16-bit signed stereo PCM. To
  eliminate static bursts from large DC bias offsets (e.g., +15,000), filter
  samples through a single-pole IIR DC blocker
