...
updated_at: 2026-09-16T00:32:28.469284+00:00
confidence: 1.0
---

# Project Context: Antigravity Plugin

Plugin packaging rules, skills, sidecars (`sidecar.json`), and lifecycle
hooks at `/workspace/antigravity-plugin`. Registered in
`~/.gemini/config/plugins.json`, tracking `main` at
`git@github.com:klinker41/antigravity-plugin.git`, and vendoring skills via
`vendor/agent-skills`.

## Invariants & Rules
- **Web Applications:** Bun runtime/package manager, Hono framework,
...
- **Development Standards:** Use `Model: "flash"` for coding/testing subagents
  (`rules/coding-subagent-model.md`). Enforce kebab-case names, minimal
  diffs, no duplication, README updates, and strict 80-character Markdown
  wrapping.
...
- **Nightly Maintenance:** Dreamer at 00:00 (tracks step watermarks in
  `state.json`, checks existing topics via `memory_utils` to avoid
  duplicates, skips subagents and `rules/*.md`, purges ephemeral sessions);
  tiered compaction at 00:30; Git sync at 01:00 local time.

## Shared Utilities (`utils/memory_utils.py`)
- **Runtime Discovery & Topics:** Probes active `cli.log` and runtime state
  for live `ANTIGRAVITY_LS_ADDRESS` and `ANTIGRAVITY_CSRF_TOKEN` when env vars
  are stale; provides `get_existing_topics` topic deduplication.
...
## Sidecars & Skills
- **Scheduled Sidecars (`sidecar.json`):**
  Recurring `agentapi new-conversation` tasks (supporting `--model` overrides).
...
- **Slack Integration:** `sidecars/slack-chat/` bridges Slack Socket Mode to
  ...
  with `conversations_replies` backfill. Validated by
  `prep-slack-chat-sidecar` (`SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`);
  `notify-the-user` sends webhooks.
