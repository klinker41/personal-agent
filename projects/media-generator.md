...
- **API Resilience & Utilities**: Gemini client
  (`backend/src/services/gemini.ts`) implements `postWithRetry` for HTTP 429
  and 5xx errors with exponential backoff, jitter, `Retry-After` parsing, and
  `AbortSignal.timeout`. XML escaping is centralized in
  `backend/src/services/rss.ts` (`escapeXml`) across RSS, FFmpeg, and
...
- **Workspace Tooling & Commands**: Root scripts coordinate build, lint,
  typecheck, and testing across workspaces using Bun and Vite.
  - Backend tests: `cd backend && NODE_ENV=test bun test` (do not use verbose
    flags).
  - Frontend tests: `vitest run`.
  - Typechecking: `cd backend && bunx tsc --noEmit` and
