...
updated_at: 2026-09-21T00:35:18.096703+00:00
confidence: 0.95
---
...
- **Core Runtime & Stack**: Native Bun runtime with TypeScript, Hono web
  framework, Zod validation schemas, native `bun:sqlite` (WAL mode, foreign
  keys), native WebSockets (`createBunWebSocket`), and `bun test`.
...
- **Python Emulation Environment**: Dedicated uv-managed virtual environment
  (`.venv` via `/usr/bin/python3`), PyBoy 2.7.0 (`window="null"`, `speed=1`,
  `sound_emulated=True`), pillow 12.3.0, numpy 2.5.3, pysdl2 0.9.17, and
  pysdl2-dll 2.32.10 (Linux x86_64 SDL2 binaries). SDL warnings suppressed via
  `PYTHONWARNINGS="ignore:Using SDL2 binaries"`. Hybrid manifest
  (`pyproject.toml` PEP 621 declarative and `requirements.txt`).
- **Media Dependencies**: Static FFmpeg binary at
  `/home/developer/.local/bin/ffmpeg` (BtbN GPL build with `drawtext`, `tee`,
  `apad`, `aresample`, `libx264`) and bundled TrueType fonts in `assets/fonts/`
  (fallback to `Inter-Regular.ttf` and `Inter-Bold.ttf`).
- **Container Environment**: Multi-stage Dockerfile (`oven/bun:1-debian`
  builder, standalone Bun and Python runtime) bound to port 4401. `ROMS_DIR` and
  `DATA_DIR` fall back to local `./roms` and `./data` to avoid root container
  permission requirements.

## Emulator Subsystem & IPC (`src/emulator/`)

- **Engine Selection**: Evaluated gen1recomp (recreation engine); retained
  PyBoy for deterministic frame-freezing, cycle-accurate save-state
  checkpointing, and headless execution.
- **Dual Unix Domain Socket IPC**:
  - *Control Socket (`/tmp/llm-pokemon-control.sock`)*: Bidirectional NDJSON RPC
    ...
    Unlinks stale sockets on startup. Preserves correlation 
<truncated 5540 bytes>
odology**: 32+ test files executing under `bun test`
...
- **Test Mocks (`tests/mocks/`)**:
  - `MockWorkerServer`: Dual Unix domain socket bridge (NDJSON RPC + 16-byte
    PKMN frames), preserves correlation IDs on errors, supports camelCase and
    snake_case properties, unlinks stale sockets on startup.
  ...
  - `MockEmulatorAdapter`: In-process deterministic RGB24 frame generation and
    pure TypeScript PNG encoding via `Bun.deflateSync`. Uses bundled
    `default_rom.gb` to test lifecycles without commercial ROMs.
