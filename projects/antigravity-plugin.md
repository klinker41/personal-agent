...
updated_at: 2026-09-11T00:31:05.470391+00:00
confidence: 1.0
---
...
- **Git & Commits:** Mandatory `self-review-commit` pre-commit loop, zero
  secrets in diffs, and explicit user confirmation for `git push`
  (`rules/self-review-before-commit.md`, `rules/no-secrets-in-commits.md`,
  `rules/git-push.md`).
- **Development Standards:** Minimal diffs, no code duplication, kebab-case
...
- **Hybrid Pipeline & Hooks:** Deterministic Python handles secret scrubbing,
...
- **Nightly Maintenance:** Dreamer at 00:00 (tracks step watermarks in
  `state.json`, checks `memory_utils.get_existing_topics` to prevent duplicate
...
- **Dynamic LS Discovery:** Probes runtime state and active `cli.log` for
  ...
  stale.
- **AgentApiBridge:** Subprocess runner for `agentapi`. Clears caller env vars
  (`ANTIGRAVITY_SOURCE_METADATA`, `ANTIGRAVITY_CONVERSATION_ID`,
  `ANTIGRAVITY_TRAJECTORY_ID`) to avoid project mismatch, fetches web hub CSRF
...
- **Slack Integration:** `sidecars/slack-chat/` bridges Slack Socket Mode to
  ...
  backfill via `conversations_replies`. Validated by
  `skills/prep-slack-chat-sidecar` (`SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`).
  `notify-the-user` sends Slack webhooks.
- **Workflow & Chat Skills:** `self-review-commit` (runs a `Model="pro"`
  subagent reviewer); `ollama-chat` (queries Gemma models at
  `https://ollama.klinker-cabin.computer` with `$OLLAMA_API_KEY`);
